...
export class HeaderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.router.events
      ...
      .subscribe((event) => {
        const targetURL = this.concatURL(event);
        if (targetURL !== 'home') {
  ...
  openNewTab(tab: Tab) {
    ...
    this.globalNavService._tabNameObject.subscribe((tabName) => {
      ...
      for (const tabItem of this.tabs) {
        console.log('---FOR LOOP Tab--- ' + tabItem);
        if (tabName != null) {
          if (tabItem.url === tabName.url) {
            tabItem.name = tabName.value;
          }
        }
      }
    });
  ...
  concatURL(urlSegment: UrlSegment[]): string {
    let joinedURL = '';
    urlSegment.forEach((str) => {
      joinedURL += str.toString() + '/';
    });
